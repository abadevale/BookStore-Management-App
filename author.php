...
#if there is no author id set
if (!isset($_GET['id'])) {
    // Redirect to index.php if no author id is provided
    header("Location: index.php");
    exit;
}

#capturing author id from the GET request
$id = $_GET['id'];

#db connection file responsible for database connection
include "db_conn.php";

#retrieve all books using the get_books_by_author function and store the result in $books
...
#retrieve all categories using the get_all_categories function and store the result in $categories
...
<!-- Navbar for the site with div container to customize using CSS and JavaScript -->
...
<div class="d-flex pt-3">
    <?php if ($books == 0) {?>
        ...
        <div class="alert alert-warning text-center p-5" role="alert">
            There are no books in the database for this author.
        </div>
    <?php }else{ ?>
    ...
    <div class="category">
        ...
        <div class="list-group">
            ...
            <?php }else{ ?>
                ...
                <?php foreach ($categories as $category) { ?>
                    <a href="category.php?id=<?= $category['id'] ?>" class="list-group-item list-group-item-action active"><?= $category['name'] ?></a>
                <?php } ?>
            <?php } ?>
        </div>
